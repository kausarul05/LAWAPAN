...
import { ArrowLeft, Plus, Pencil, X, Loader } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { getBankDetails, createBankDetails, updateBankDetails } from '@/components/lib/apiClient';
...
const BankDetailsView = () => {
  ...
  const handleEditClick = () => {
    setFormData({
      ...
      bankholder_name: bankDetails.bankholder_name || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing && bankDetails?._id) {
        console.log('📝 Updating bank details:', payload);
        response = await updateBankDetails(bankDetails._id, payload);
      } else {
      ...
      if (response.success) {
        alert(isEditing ? 'Bank details updated successfully!' : 'Bank details added successfully!');
        setShowModal(false);
        resetForm();
        fetchBankDetails();
      } else {
    ...
    <div className="p-6 bg-[#F4F7FA] min-h-screen">
      ...
      {bankDetails ? (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-6 border-r border-b border-gray-100">
              ...
              <p className="font-bold text-gray-800">{bankDetails.account_number}</p>
            </div>
            <div className="p-6 border-b border-gray-100">
              <p className="text-sm text-gray-400 mb-1">Routing Number</p>
              <p className="font-bold text-gray-800">{bankDetails.routing_number}</p>
            </div>
            <div className="p-6 border-r border-b border-gray-100">
              <p className="text-sm text-gray-400 mb-1">Bank Name</p>
              <p className="font-bold text-gray-800">{bankDetails.bank_name}</p>
            </div>
            <div className="p-6 border-b border-gray-100">
              <p className="text-sm text-gray-400 mb-1">Bankholder Name</p>
              <p className="font-bold text-gray-800">{bankDetails.bankholder_name}</p>
            </div>
            <div className="p-6 col-span-1 md:col-span-2">
              <p className="text-sm text-gray-400 mb-1">Bank Address</p>
              <p className="font-bold text-gray-800">{bankDetails.bank_address}</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">No bank account added yet</p>
          <button 
            onClick={handleAddClick}
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white">
                <h2 className="text-xl font-bold text-gray-900">
                  {isEditing ? 'Edit Bank Details' : 'Add Bank Account'}
                </h2>
                <button 
              ...
              <div className="p-6 space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border ${formErrors.account_number ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-[#036BB4]`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border ${formErrors.routing_number ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-[#036BB4]`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border ${formErrors.bank_name ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-[#036BB4]`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border ${formErrors.bankholder_name ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-[#036BB4]`}
                  />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full px-4 py-3 border ${formErrors.bank_address ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-[#036BB4] resize-none`}
                  />
