    ...
    >
      <div className="flex flex-col h-full overflow-y-auto no-scrollbar">
                ...
                {settingsDropdown.map(({ name, href, icon: Icon }) => {
                  ...
                  return (
                    <Link
                      key={name}
                      href={href}
                      className={`flex items-center px-3 py-2 rounded-md text-xs font-medium transition-all hover:bg-white ${isActive
                        ? "shadow-md"
                        : "text-gray-500"
                        }`}
                      style={isActive ? { backgroundColor: '#036BB4', color: '#ffffff' } : {}}
        ...
        <div className="p-3 border-t border-[#D6D6D6] mt-auto">
          <button onClick={handleLogout} className={`flex items-center h-12 w-full text-red-500 hover:bg-red-50 rounded-lg transition-all group relative ${!isOpen ? "justify-center" : "px-3"
            }`}>
